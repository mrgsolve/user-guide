...
```{r}
#| fig-height: 4
#| label: event-usage-example
#| fig-cap: Example simulation output from an event object. 
mod <- house(end = 72) 
...
Columns can be removed from event objects using `select()`

```{r}
ev(amt = 100, WT = 50, AGE = 12) %>% select(-WT)
```

Rows can be removed from event objects with `filter()`
