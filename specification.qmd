...
Plugins are listed and discussed in more detail in @sec-plugins.
...
See also: @sec-block-preamble.

### `$THETA`  \ {#sec-block-theta}
...
See also: @sec-block-param.


### `$NMXML`  \ {#sec-block-nmxml}
...
If your model run is 1000 and the `.xml` file is located in `1000/1000.xml`, 
use this syntax
...
If your model run is 1000 and the `.xml` file is located in some other 
location use the `path` argument to specify the _complete_ path to the `.xml`
file
...
This latter syntax may be needed when using PsN and output files are organized
in a way different from the previous example.


**Details**

From the NONMEM run, `THETA` will be imported into your
parameter list (see @sec-block-param and @sec-component-param), `OMEGA` will 
be captured as an `$OMEGA` block (@sec-block-omega) and `SIGMA` will be 
captured as a `$SIGMA` block (@sec-block-sigma).  Users may optionally omit 
...
you will need to provide the `path` argument, which should be the path to the 
...
take a look at @sec-block-nmext for more discussion and examples. 

Starting with version 1.0.8, when the NONMEM run is found using the `run` +
`project` arguments, you can pass `run = "@cppstem"` and mrgsolve will look for
the NONMEM run number that matches the stem of the current mrgsolve file. For
example, if you are coding a model to match NONMEM run `101.ctl`, then call your
mrgsolve model file `101.cpp` or `101.mod`; just match the file stem. Then write

```{c, eval = FALSE, filename = "101.cpp"}
$NMXML 
run = "@cppstem"
project = <nonmem-run-directory>
```

mrgsolve will check the stem of the mrgsolve model file and  look for the NONMEM
`.xml` file in
...
See also: @sec-block-nmext.


### `$NMEXT`  \ {#sec-block-nmext}
...
If your model run is 1000 and the `.ext` file is located in `1000/1000.ext`, 
use this syntax
...
If your model run is 1000 and the `.ext` file is located in some other 
location use the `path` argument to specify the _complete_ path to the `.ext`
file
...
See the `$NMXML` topic in @sec-block-nmxml for an example of equivalent 
functionality.
...
Starting with version 1.0.8, when the NONMEM run is found using the `run` +
`project` arguments, you can pass `run = "@cppstem"` and mrgsolve will look for
the NONMEM run number that matches the stem of the current mrgsolve file. 
See details and example in @sec-block-nmxml.
...
See also: @sec-block-nmxml.

### `$INCLUDE`  \ {#sec-block-include}
...
Objects inside `$ENV` can be utilized in different C++ functions 
(see @sec-block-plugin) or other parts of the simulation process.

Objects inside `$ENV` will also be made available when parsing different
parts of the model specification file. For example, we can initialize an
`$OMEGA` block with the `Sigma`  object in the `$ENV` block example above:
...
You can also make your model specification file depend on an R package; 
for example, if you want to use the `here` package to render a path to 
your NONMEM model
...
See also `env_ls()` to list the names of variables in the model environment, 
`env_update()` to change the values of objects in the model environment, 
or `env_eval` to re-evaluate the model environment code.   


## Variables and Macros {#sec-variables-macros}
...
to `SOLVERTIME` inside `$ODE`  when you invoke the `nm-vars` plugin
(@sec-plugin-nm-vars).
...
(@sec-steady-state); then it will be `true`. This variable is only 
...
Starting with mrgsolve version 1.6.1, `simeta()` is available in `$MAIN`
(`$PK)`, `$TABLE` (`$ERROR`), `$EVENT` and `$PRED` blocks. 
...
when the `Rcpp` plugin is invoked (see @sec-plugin-rcpp).
...
### `self.mtime(<time>)` \ {#sec-self.mtime}
...
Related to `self.mtime()` (@sec-self.mtime), except you can set a 
...
`THETA1`, `THETA2`, `THETA3` and so on (see @sec-block-nmxml and
@sec-block-nmext). `THETA(n)` is simply a NONMEM-oriented style for referring
to `THETAn`.
...
Starting with version 1.0.8, users have read-only access to on-diagonal elements
of $\Sigma$ through the `SIGMA()` macro. For example
...
See: @sec-block-table and also @sec-block-capture.

### `END_OF_INFUSION` \ {#sec-spec-end-of-infusion}

This is a boolean variable indicating if the current record
terminates an infusion. 
...
### `F_CMT` \ {#sec-spec-f}
For the `CMT` compartment, sets the bioavailability fraction for that 
compartment. 
...
### `Fn`
Only available when the `nm-vars` plugin (@sec-plugin-nm-vars) is invoked; 
see `F_CMT`.
...
Starting with mrgsolve version 1.6.1, `D_CMT` is available in `$MAIN`
(`$PK)`, `$TABLE` (`$ERROR`), `$EVENT` and `$PRED` blocks. 
...
Starting with mrgsolve version 1.6.1, `ALAG_CMT` is available in `$MAIN`
(`$PK)`, `$TABLE` (`$ERROR`), `$EVENT` and `$PRED` blocks. 
...
See @sec-plugin-nm-vars on the `nm-vars` plugin for details. 
...
## Derive new variables {#sec-new-variables}
...
We want `CL` to be a numeric value, so we use `double`.  To initialize a `boolean` 
...
initialized in `$PREAMBLE`, `$MAIN`, `$ODE`, `$TABLE` or `$EVENT`, mrgsolve will 
detect those declarations, and modify the code so that the variables are actually
...
a double-precision numeric variable is created (`CL`) in the `$MAIN` block. 
...
That is, `CL` is declared in `$GLOBAL` in an unnamed namespace so that variables
...
The `local_double` type is still just a double-precision variable.  The
difference is that it is protected from this re-declaration process and the
variable will be local to (in this case) the `$MAIN` block.

Similarly, use `local_int` or `local_bool` to declare a variable that will _not_
be made global by mrgsolve. One common application of `local_int` is when 
you need to write a for-loop
...
Here, we we want `i` declared locally for use in that for-loop and we use 
`local_int` to make that happen.

### Initializing and resetting 

mrgsolve takes a hands-off approach to variables created by the user; it will
not initialize or reset variables from record to record or from individual to
individual. Failing to properly initialize or reset user variables can have 
unintended consequences, especially when conditional logic is used. 

Generally, a default value for all variables should be set prior to 
conditional assignment based on some test condition. 
...
There is a plugin you can invoke which will instruct mrgsolve to scan your
code and discover variables to be declared with type `double`. See the 
autodec plugin in @sec-plugin-autodec for details.
...
using the `Rcpp` plugin (see @sec-plugins and @sec-plugin-rcpp). 
...
* We added zeros for both `$OMEGA` matrices; all the etas will be zero until we
  populate those matrices (@sec-matrix-update)
