...
## Annotated model specification {#sec-topic-annotated}
...
## Set initial conditions {#sec-topic-init}
...
## Updating parameters {#sec-topic-parameter-update}

The parameter list was introduced in @sec-component-param and the
`$PARAM` code block was shown in @sec-block-param. Once a model is compiled,
...
See @sec-topic-parameter-update-base for a more detailed discussion 
...
(parameters), see @sec-topics-time-varying and @sec-section-sequence.
...
### Updating the base parameter list {#sec-topic-parameter-update-base}
...
## Time grid objects {#sec-topic-tgrid}
...
## Individualized sampling designs {#sec-topic-designs}
...
__Call `simeta()` to resimulate `ETA`__
...
## Time varying covariates {#sec-topics-time-varying}
...
during system advance in @sec-section-sequence.
