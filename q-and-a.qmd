...
# Questions and Answers {#sec-q-and-a}
...
special plugin that will do this automatically for you; see 
@sec-plugin-cxx11.

## Can I compile my model with `C++11`?

Yes, you can do this by invoking the `CXX11` plugin (@sec-plugin-cxx11).
...
Most applications will call `self.tad()` (@sec-self.tad). For example
...
the `tad` plugin.  See @sec-plugin-tad for details.
