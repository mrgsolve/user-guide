...
__Note__: This user guide section provides some low-level information on working
with event objects to issue doses from inside your model code. The `evtools` 
plugin described in @sec-plugin-evtools provides a higher-level interface
to these objects and should be used whenever possible. 
...
### Event object public members {#sec-event-members}

Once the object is created, you can modify or access the following public 
members
...
The `evtools` plugin described in @sec-plugin-evtools provides a higher-level
interface to these objects. We recommend using the `evtools` interface whenever
possible.


### Push the evdata object back to mrgsolve
...
Again, this sequence should get called in either `$EVENT` or `$ERROR`. When that 
